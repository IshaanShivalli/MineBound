...
function Core:spawnWave(playState)
    ...
    -- Spawn Top and Bottom Lane Minions
    local m1 = Minion(self.x + spawnOffset, self.y - 32, self.owner, targetCore)
    local m2 = Minion(self.x + spawnOffset, self.y + 32, self.owner, targetCore)
...
function Core:render()
    ...
    love.graphics.printf(label, self.x - 20, self.y - 22, self.size + 40, 'center')
